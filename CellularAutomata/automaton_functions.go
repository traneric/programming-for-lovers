...
func PlayAutomaton(initialBoard GameBoard, numGens int, neighborhood string,
  ruleStrings []string) []GameBoard {
...
func UpdateBoard(currBoard GameBoard, neighborhood string,
  ruleStrings []string) GameBoard {
...
//UpdateCell takes a gameboard along with row and col indices and consults a
//collection of rule strings for a given neighborhood type to update cell
//state at given row and col indices.
func UpdateCell(board GameBoard, r, c int, neighborhood string,
  ruleStrings[]string) int {
	// range through rule strings and look for a match ... very different for
  //the two neighborhood types
...
//RuleMatch takes a GameBoard board, row/col indices r, c, a neighborhood
//parameter (vonNeumann or Moore), and a rule string.
...
func RuleMatchMoore(board GameBoard, r, c int, rule string) bool {
	...
	// see if we find a mismatch with the current rule and any of the cell
  //and its eight neighbors
	if center != string(rule[0]) || northwest != string(rule[1]) ||
  north != string(rule[2]) || northeast != string(rule[3]) ||
  east != string(rule[4]) || southeast != string(rule[5]) ||
  south != string(rule[6]) || southwest != string(rule[7]) ||
  west != string(rule[8]) {
		return false
	}
...
//InitializeBoard takes a number of rows and columns as inputs and returns a
//gameboard with appropriate number of rows and colums, where all values = 0.
