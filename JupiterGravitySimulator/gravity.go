...
//SimulateGravity takes an initial universe as well as a number of generations
//and a time interval parameter. It returns a slice of Universe objects
//corresponding to running a gravity simulation over the appropriate number of
//generations, estimated for the given time interval (in seconds).
...
//UpdateUniverse takes a Universe object and a time interval (in seconds) and
//returns the updated universe simulated after the given time interval.
func UpdateUniverse(currentUniverse Universe, time float64) Universe {
    newUniverse := CopyUniverse(currentUniverse)

    // for every body in new universe, update its fields that will change
...
//UpdateVelocity takes a Body object and a time interval in seconds.  It
//updates the velocity of that object over the time interval using Newtonian
//dynamics equations.
...
//UpdatePosition takes a Body object and a time interval in seconds.
//It updates the position of that object over the time interval using Newtonian
//dynamics equations.
...
//UpdateAcceleration takes a slice of Body objects and a Body object and
//computes the acceleration of the body object based on the force of gravity
...
//ComputeNetForce takes a slice of Body objects and a separate body object.
//It returns the net force of gravity (as an ordered pair) that the bodies
//exert on the given Body object.
func ComputeNetForce(bodies []Body, b Body) OrderedPair {
    var netForce OrderedPair

    // range over all bodies in the slice and compute the force that they
    //exert on the given body
    for i := range bodies {
        if bodies[i] != b {
...
//ComputeForce takes two bodies and computes force exerted due to gravity on
//the second object due to the first.
...
//Distance takes two position ordered pairs and it returns the distance between
//these two points in 2-D space.
...
//CopyUniverse takes a Universe object and produces a new Universe object with
//all fields copied over.
func CopyUniverse(currentUniverse Universe) Universe {
    ...
    // this wouldn't duplicate the bodies ... it would give me the same bodies:
    ...
    for i := range currentUniverse.bodies {
        var b Body
        // set the fields of b equal to those of the current body
        //(currentUniverse.bodies[i])
